...
def handle_cleanup():
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        invalid_ids = []
        for img_id, filepath in rows:
            # 这里的 filepath 应该是数据库存的原始绝对路径
            if not os.path.exists(filepath):
                invalid_ids.append(img_id)
        
        if invalid_ids:
            print(f"发现 {len(invalid_ids)} 条失效路径，正在清理...")
...
def main():
    ...
    # --- 新增：cleanup 命令 ---
    parser_cleanup = subparsers.add_parser("cleanup", help="Manually trigger database self-check and cleanup.")
    ...
    if args.command == "index":
        ...
        handle_search(args)
    elif args.command == "cleanup": # --- 新增分支 ---
        handle_cleanup()
